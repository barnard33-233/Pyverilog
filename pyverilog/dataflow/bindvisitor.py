...
class BindVisitor(NodeVisitor):
    ...
    def _visit_Instance_array(self, node):
        if node.name == "":
            raise verror.FormatError(
                "Module %s requires an instance name" % node.module
            )
    ...
    def _visit_Instance_body(self, node, nodename, arrayindex=None):
        ...
        for paramnames_i, param in enumerate(node.parameterlist):
            ...
            paramname = (
                paramnames[paramnames_i] if param.paramname is None else param.paramname
            )
        ...
        for ioport_i, port in enumerate(node.portlist):
            if port.portname is not None and not (port.portname in ioports):
                raise verror.FormatError(
                    "No such port: %s in %s" % (port.argname.name, nodename)
                )
    ...
    def _visit_Instance_primitive(self, node, arrayindex=None):
        ...
        if primitive_type == None:
            right = (
                Pointer(node.portlist[1].argname, IntConst("0"))
                if arrayindex is None
                else Pointer(node.portlist[1].argname, IntConst(str(arrayindex)))
            )
        elif primitive_type == Unot:
            right = (
                Ulnot(Pointer(node.portlist[1].argname, IntConst("0")))
                if arrayindex is None
                else Ulnot(Pointer(node.portlist[1].argname, IntConst(str(arrayindex))))
            )
        else:
            concat_list = (
                [Pointer(p.argname, IntConst("0")) for p in node.portlist[1:]]
                if arrayindex is None
                else [
                    Pointer(p.argname, IntConst(str(arrayindex)))
                    for p in node.portlist[1:]
                ]
            )
    ...
    def visit_Always(self, node):
        ...
        current = self.stackNextFrame(
            label, "always", generate=self.frames.isGenerate(), always=True
        )
        ...
        (
            ...
            reset_bit,
            senslist,
        ) = self._createAlwaysinfo(node, current)

        self.frames.setAlwaysInfo(
            clock_name,
            clock_edge,
            clock_bit,
            reset_name,
    ...
    def visit_IfStatement(self, node):
        ...
        if (
            self.frames.isGenerate()
            and not self.frames.isAlways()
            and not self.frames.isInitial()
            and not self.frames.isFunctioncall()
            and not self.frames.isTaskcall()
            and not self.frames.isFunctiondef()
            and not self.frames.isTaskdef()
        ):
    ...
    def visit_WhileStatement(self, node):
        ...
        while True:
            ...
            if not isinstance(rslt, DFEvalValue):
                raise verror.FormatError(
                    (
                        "Can not process the while-statement. "
    ...
    def visit_GenerateStatement(self, node):
        ...
        current = self.stackNextFrame(label, "generate", generate=True)
    ...
    def visit_NonblockingSubstitution(self, node):
        if self.frames.isForpre() or self.frames.isForpost():
            raise verror.FormatError(
                ("Non Blocking Substitution is not allowed" "in for-statement")
            )
        if self.frames.isFunctioncall():
            raise verror.FormatError(
                "Non Blocking Substitution is not allowed in function"
            )
        self.addBind(
            node.left, node.right, self.frames.getAlwaysStatus(), "nonblocking"
        )
    ...
    def renameVar(self, name):
        renamedvar = name[:-1] + ScopeLabel(
            "_rn" + str(self.renamecnt) + "_" + name[-1].scopename, "signal"
        )
        self.renamecnt += 1
        return renamedvar
    ...
    def makeConstantTerm(self, name, node, scope):
        ...
        msb = (
            ...
            else self.makeDFTree(node.width.msb, scope)
        )
        lsb = (
            DFIntConst("0")
            if node.width is None
            else self.makeDFTree(node.width.lsb, scope)
        )
        ...
        for t in termtype:
            if t == "Input":
                ...
                portarg = (
                    port.argname
                    if arrayindex is None
                    else Pointer(port.argname, IntConst(str(arrayindex)))
                )
    ...
    def makeDFTree(self, node, scope):
        ...
        if isinstance(node, Pointer):
            ...
            if (
                isinstance(var_df, DFTerminal)
                and self.getTermDims(var_df.name) is not None
            ):
    ...
    def removeOverwrappedCondition(self, tree, current_bindlist, scope):
        ...
        condlist, flowlist = self.getCondflow(scope)
        (merged_tree, rest_condlist, rest_flowlist, match_flowlist) = (
            self.diffBranchTree(merged_tree, condlist, flowlist)
        )
    ...
    def resolveBlockingAssign(self, tree, scope):
        ...
        if isinstance(tree, DFPartselect):
            ...
            if isinstance(resolved_var, DFBranch):
                return reorder.insertPartselect(
                    resolved_var, resolved_msb, resolved_lsb
                )
        ...
        if isinstance(tree, DFPointer):
            ...
            if (
                isinstance(tree.var, DFTerminal)
                and self.getTermDims(tree.var.name) is not None
            ):
                ...
                for bind in current_bindlist:
                    new_tree = DFBranch(
                        DFOperator((bind.ptr, resolved_ptr), "Eq"), bind.tree, new_tree
                    )
                print(
                    (
                        "Warning: "
        ...
        raise verror.FormatError(
            "unsupported DFNode type: %s %s" % (str(type(tree)), str(tree))
        )
    ...
    def getFitTree(self, bindlist, msb, lsb):
        ...
        for bind in bindlist:
            if bind.msb is None and bind.lsb is None:
                return bind.tree
            if (
                self.optimize(bind.msb) == optimized_msb
                and self.optimize(bind.lsb) == optimized_lsb
            ):
                return bind.tree
    ...
    def getMergedTree(self, bindlist):
        ...
        def bindkey(x):
            ...
            length = (
                abs(self.optimize(term.msb).value - self.optimize(term.lsb).value) + 1
            )
        ...
        for bind in sorted(bindlist, key=bindkey):
            ...
            if last_ptr != (
                -1 if not isinstance(bind.ptr, DFEvalValue) else bind.ptr.value
            ):
                continue
            if last_msb + 1 < lsb:
    ...
    def getDst(self, left, scope):
        ...
        raise verror.FormatError(
            "unsupported AST node type: %s %s" % (str(type(left)), str(left))
        )

    def setDataflow(
        self, dst, raw_tree, condlist, flowlist, alwaysinfo=None, bindtype=None
    ):
        ...
        for name, msb, lsb, ptr, part_msb, part_lsb in dst:
            ...
            if alwaysinfo is not None:
                self.setNonblockingAssign(
                    name, dst, raw_tree, msb, lsb, ptr, part_msb, part_lsb, alwaysinfo
                )

    def setDataflow_rename(
        self, dst, raw_tree, condlist, flowlist, scope, alwaysinfo=None
    ):
    ...
    def setNonblockingAssign(
        self, name, dst, raw_tree, msb, lsb, ptr, part_msb, part_lsb, alwaysinfo
    ):
        tree = raw_tree
        if len(dst) > 1:
    ...
    def setRenamedTree(self, renamed_dst, raw_tree, alwaysinfo):
        for name, msb, lsb, ptr, part_msb, part_lsb in renamed_dst:
            ...
            if len(renamed_dst) > 1:
                tree = reorder.reorder(DFPartselect(tree, part_msb, part_lsb))
    ...
    def setRenamedFlow(
        self, dst, renamed_dst, condlist, flowlist, scope, alwaysinfo=None
    ):
        term_i = 0
        for name, msb, lsb, ptr, part_msb, part_lsb in dst:
            renamed_term = DFTerminal(renamed_dst[term_i][0])
            renamed_bind = self.makeBind(
                name,
                msb,
                lsb,
                ptr,
    ...
    def makeBind(
        ...
        if len(current_bindlist) > 0:
            for current_bind in current_bindlist:
                if (
                    current_bind.msb == msb
                    and current_bind.lsb == lsb
        ...
        match_flowlist = ()
        if current_msb == msb and current_lsb == lsb and current_ptr == ptr:
            (rest_tree, rest_condlist, rest_flowlist, match_flowlist) = (
                self.diffBranchTree(current_tree, condlist, flowlist)
            )
