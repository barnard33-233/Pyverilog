...
class VerilogParser(object):
    ...
    def p_param_signed(self, p):
        ...
        paramlist = [
            Parameter(rname, rvalue, signed=True, lineno=p.lineno(2))
    ...
    def p_lparam_signed(self, p):
        ...
        paramlist = [
            Localparam(rname, rvalue, signed=True, lineno=p.lineno(2))
            for rname, rvalue in p[3]
        ]
        p[0] = Decl(tuple(paramlist), lineno=p.lineno(1))
        p.set_lineno(0, p.lineno(1))
    ...
    def p_lparam_width(self, p):
        ...
        paramlist = [
            Localparam(rname, rvalue, p[2], lineno=p.lineno(3))
            for rname, rvalue in p[3]
        ]
        p[0] = Decl(tuple(paramlist), lineno=p.lineno(1))
        p.set_lineno(0, p.lineno(1))
    ...
    def p_param_signed_width(self, p):
        ...
        paramlist = [
            Parameter(rname, rvalue, p[3], signed=True, lineno=p.lineno(3))
    ...
    def p_lparam_signed_width(self, p):
        ...
        paramlist = [
            Localparam(rname, rvalue, p[3], signed=True, lineno=p.lineno(3))
            for rname, rvalue in p[4]
        ]
        p[0] = Decl(tuple(paramlist), lineno=p.lineno(1))
        p.set_lineno(0, p.lineno(1))
    ...
    def typecheck_ioport(self, sigtypes):
        if (
            "input" not in sigtypes
            and "output" not in sigtypes
            and "inout" not in sigtypes
        ):
    ...
    def p_ioport_dimensions(self, p):
        ...
        p[0] = self.create_ioport(
            p[1], p[3], width=p[2], dimensions=p[4], lineno=p.lineno(3)
        )
        p.set_lineno(0, p.lineno(1))
    ...
    def create_decl(self, sigtypes, name, width=None, dimensions=None, lineno=0):
        ...
        if "supply0" in sigtypes:
            decls.append(
                Supply(
                    name=name,
                    value=IntConst("0", lineno=lineno),
                    width=width,
                    signed=signed,
                    lineno=lineno,
                )
        ...
        if "supply1" in sigtypes:
            decls.append(
                Supply(
                    name=name,
                    value=IntConst("1", lineno=lineno),
                    width=width,
                    signed=signed,
                    lineno=lineno,
                )
    ...
    def p_decl(self, p):
        ...
        for rname, rdimensions in p[2]:
            decllist.extend(
                self.create_decl(
                    p[1], rname, dimensions=rdimensions, lineno=p.lineno(2)
                )
            )
    ...
    def p_declassign_element(self, p):
        ...
        assign = Assign(
            Lvalue(Identifier(p[1], lineno=p.lineno(1)), lineno=p.lineno(1)),
            p[3],
            lineno=p.lineno(1),
        )
        p[0] = (p[1], assign)
    ...
    def p_declassign_element_delay(self, p):
        ...
        assign = Assign(
            Lvalue(Identifier(p[2], lineno=p.lineno(1)), lineno=p.lineno(2)),
            p[5],
            p[1],
            p[4],
            lineno=p.lineno(2),
    ...
    def p_integerdecl(self, p):
        ...
        intlist = [
            Integer(
                rname,
                Width(
                    msb=IntConst("31", lineno=p.lineno(2)),
    ...
    def p_integerdecl_signed(self, p):
        ...
        intlist = [
            Integer(
                rname,
                Width(
                    msb=IntConst("31", lineno=p.lineno(3)),
    ...
    def p_partselect_plus(self, p):
        ...
        inc_value = Minus(p[5], IntConst("1"))
        p[0] = Partselect(
            p[1], p[3], Plus(p[3], inc_value, lineno=p.lineno(1)), lineno=p.lineno(1)
        )
        p.set_lineno(0, p.lineno(1))
    ...
    def p_partselect_minus(self, p):
        ...
        dec_value = Minus(p[5], IntConst("1"))
        p[0] = Partselect(
            p[1], p[3], Minus(p[3], dec_value, lineno=p.lineno(1)), lineno=p.lineno(1)
        )
        p.set_lineno(0, p.lineno(1))
    ...
    def p_namedblock_statement(self, p):
        ...
        if isinstance(p[1], Decl):
            for r in p[1].list:
                if (
                    not isinstance(r, Reg)
                    and not isinstance(r, Wire)
                    and not isinstance(r, Integer)
                    and not isinstance(r, Real)
                    and not isinstance(r, Parameter)
                    and not isinstance(r, Localparam)
                ):
                    raise ParseError("Syntax Error")
        p[0] = p[1]
        p.set_lineno(0, p.lineno(1))
    ...
    def p_genvar(self, p):
        ...
        p[0] = Genvar(
            name=p[1],
            width=Width(
                msb=IntConst("31", lineno=p.lineno(1)),
    ...
    def p_funcvardecl(self, p):
        ...
        if isinstance(p[1], Decl):
            for r in p[1].list:
                if (
                    not isinstance(r, Input)
                    and not isinstance(r, Reg)
                    and not isinstance(r, Integer)
                ):
                    raise ParseError("Syntax Error")
        p[0] = p[1]
    ...
    def p_scope(self, p):
        ...
        p[0] = IdentifierScope(
            scope + (IdentifierScopeLabel(p[1].name, lineno=p.lineno(1)),),
            lineno=p.lineno(1),
        )
        p.set_lineno(0, p.lineno(1))
    ...
    def p_single_statement_delays(self, p):
        ...
        p[0] = SingleStatement(
            DelayStatement(p[2], lineno=p.lineno(1)), lineno=p.lineno(1)
        )
        p.set_lineno(0, p.lineno(1))
        p[0].end_lineno = p.lineno(3)
