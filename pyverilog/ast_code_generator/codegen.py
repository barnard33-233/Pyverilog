...
class ASTCodeGenerator(ConvertVisitor):
    ...
    def visit_ModuleDef(self, node):
        ...
        template = self.get_template(filename)
        paramlist = (
            self.indent(self.visit(node.paramlist))
            if node.paramlist is not None
            else ""
        )
        portlist = (
            self.indent(self.visit(node.portlist)) if node.portlist is not None else ""
        )
        template_dict = {
            ...
            "portlist": portlist,
            "items": (
                [self.indent(self.visit(item)) for item in node.items]
                if node.items
                else ()
            ),
        }
    ...
    def visit_Ioport(self, node):
        ...
        template_dict = {
            ...
            "signed": node.first.signed
            or (node.second is not None and node.second.signed),
            "dimensions": (
                ""
                if node.first.dimensions is None
                else self.visit(node.first.dimensions)
            ),
        }
        rslt = template.render(template_dict)
        return rslt
    ...
    def visit_Parameter(self, node):
        ...
        template_dict = {
            ...
            "width": (
                ""
                if node.width is None or (value.startswith('"') and value.endswith('"'))
    ...
    def visit_Sens(self, node):
        ...
        template_dict = {
            ...
            "type": (
                node.type if node.type == "posedge" or node.type == "negedge" else ""
            ),
        }
        rslt = template.render(template_dict)
        return rslt
    ...
    def visit_IfStatement(self, node):
        ...
        template = self.get_template(filename)
        true_statement = (
            "" if node.true_statement is None else self.visit(node.true_statement)
        )
        false_statement = (
            "" if node.false_statement is None else self.visit(node.false_statement)
        )
        template_dict = {
            "cond": del_paren(self.visit(node.cond)),
    ...
    def visit_ForStatement(self, node):
        ...
        template_dict = {
            ...
            "cond": (
                "" if node.cond is None else del_space(del_paren(self.visit(node.cond)))
            ),
            "post": (
                ""
                if node.post is None
                else del_space(self.visit(node.post).replace(";", ""))
            ),
    ...
    def visit_Case(self, node):
        ...
        condlist = (
            ...
            else [del_paren(self.visit(c)) for c in node.cond]
        )
        cond = []
        for c in condlist:
            cond.append(c)
    ...
    def visit_ParamArg(self, node):
        ...
        template_dict = {
            ...
            "argname": (
                "" if node.argname is None else del_paren(self.visit(node.argname))
            ),
        }
        rslt = template.render(template_dict)
        return rslt
    ...
    def visit_GenerateStatement(self, node):
        filename = getfilename(node)
        template = self.get_template(filename)
        template_dict = {"items": [self.visit(item) for item in node.items]}
        rslt = template.render(template_dict)
        return rslt
